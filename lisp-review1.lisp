...
(defun super-delete (el lst &key (testp #'eql))
  ...
  (progn
    ...
    (setf lst (filter-first-non-el el lst testp))
    (cond
      ((null lst) lst)
      ((funcall testp (cdr lst) el)
       (setf (cdr lst) ())
       (super-delete-helper-l el lst testp))
      (t (super-delete-helper-r el lst testp)
	 (super-delete-helper-l el lst testp)))
    lst))

(defun super-delete-helper-r (el prev testp)
  "helper for recursion to the right, assumes (cdr prev) is not an el"
  (let ((cur (cdr prev)))
    (cond
      ((atom cur) ())
      ((null cur) ())
      ((funcall testp (car cur) el)
       (setf (cdr prev) (cdr cur))
       (super-delete-helper-r el prev testp))
      ((funcall testp (cdr cur) el)
       (setf (cdr cur) ())
       (super-delete-helper-l el cur testp))
      (t (super-delete-helper-r el cur testp)
	 (super-delete-helper-l el cur testp)))))

(defun super-delete-helper-l (el prev testp)
   ...
   (let ((cur (car prev)))
     (cond
      ...
      ((funcall testp (car cur) el)
       ...
       (super-delete-helper-l el prev testp))
      ((funcall testp (cdr cur) el)
       (setf (cdr cur) ())
       (super-delete-helper-l el cur testp))
      (t (super-delete-helper-r el cur testp)
...
(defun filter-first-non-el (el lst testp)
  "filter the list until the first non-el found"
  (cond
    ((null lst) ()) ;if el and lst are both ()
    ((funcall testp (car lst) el)
     (filter-first-non-el el (cdr lst) testp))
    (t lst)))
