...
(defun super-delete (elt list &key (test #'eql))
  ...
  (progn
    ...
    (setf list (filter-first-non-elt elt list test))
    (cond
      ((null list) list)
...
(defun filter-first-non-elt (elt list test)
  "filter the list until the first non-elt found"
  (cond
    ((null list) ()) ;if elt and list are both ()
    ((funcall test (car list) elt)
     (filter-first-non-elt elt (cdr list) test))
    (t list)))
